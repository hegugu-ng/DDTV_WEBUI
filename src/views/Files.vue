    ...
    <div class="body-contain--nav">
      ...
      <div v-else>
        ...
        <span class="nav-item" v-for="(item, index) in table.FilesRoot" :key="index">
          <span v-if="index + 1 === table.FilesRoot.length" class="is-disable-nav nav-item-title">{{ item }}</span>
            ...
            <td class="table-td row-icon-name" @click="GoFiles(item)">
              <div class="name-list">
                ...
                <img
                  ...
                  class="icon-list"
                />
                <img v-else :src="require('../assets/img/txt.png')" :alt="item.type" class="icon-list" />
...
export default {
  ...
  methods: {
    ...
    handleCheckedFilesChange: function (index, selent) {
      ...
      if (this.table.checked.length === this.table.table_body.length && this.table.table_body.length > 0) {
