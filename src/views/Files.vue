	...
	<div class="file">
		<div class="body-contain--nav">
			...
			<div v-else>
				...
				<span
					class="nav-item"
					v-for="(item, index) in table.FilesRoot"
					:key="index"
				>
					<span
						v-if="index + 1 == table.FilesRoot.length"
						class="is-disable-nav nav-item-title"
						>{{ item }}</span
					>
					<span v-else class="nav-item-title" @click="GoToRoot(item)">{{
						item
					}}</span>
					<span v-if="index + 1 != table.FilesRoot.length" class="nav-item-sep"
						>&gt;</span
					>
				</span>
			</div>
				...
				<thead>
					<tr class="table-header-row">
						<th class="table-header-select table-selent">
							<el-checkbox
								v-model="table.table_head.selent"
								:indeterminate="table.table_head.isIndeterminate"
								@change="handleCheckAllChange"
							></el-checkbox>
						</th>
						<th class="table-header-th table-td is-name">文件名称</th>
						<th class="table-header-th table-td cursor-p">修改日期</th>
						<th class="table-header-th table-td cursor-p">大小</th>
					</tr>
				</thead>
			</table>
		</div>
			...
			<table class="table-body-table table-all">
				...
				<tbody>
					<tr
						...
						v-for="item in table.table_body"
						:key="item.index"
						:data-id="item.index"
						:index="item.index"
					>
						<td class="row-checkbox table-selent">
							<el-checkbox
								v-model="item.selent"
								@change="handleCheckedFilesChange(item.index, item.selent)"
							></el-checkbox>
						</td>
						<td class="table-td row-icon-name" @click="GoFiles(item)">
							<div class="name-list">
								...
								<img
									v-else
									:src="require('../assets/img/txt.png')"
									:alt="item.type"
									class="icon-list"
								/>
...
export default {
	...
	methods: {
		GoToNext: async function () {
			...
			let i = 1;
			for (let root of this.table.FilesRoot) {
		...
		GoFiles: async function (item) {
			...
			if (item.type == "folder") {
				...
				window.open(
					host + "/api/File_GetFile?FileName=" + FileName + "&CMD=File_GetFile",
				);
		...
		handleCheckAllChange: function (val) {
			if (val) {
				for (let item of this.table.table_body) {
					item.selent = true;
		...
		handleCheckedFilesChange: function (index, selent) {
			if (selent) {
				...
				for (let item of this.table.checked) {
					if (item.index == index) {
			...
			if (
				this.table.checked.length == this.table.table_body.length &&
				this.table.table_body.length > 0
			) {
				(this.table.table_head.selent = true),
					(this.table.table_head.isIndeterminate = false);
			}
			if (
				this.table.checked.length < this.table.table_body.length &&
				...
				this.table.table_head.isIndeterminate = true;
